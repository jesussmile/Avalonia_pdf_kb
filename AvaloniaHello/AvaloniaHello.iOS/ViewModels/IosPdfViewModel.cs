...
namespace AvaloniaHello.iOS.ViewModels;

internal sealed class IosPdfViewModel : ViewModelBase, IPdfDocumentViewModel, IDisposable
{
    private readonly string _filePath;
    private readonly Action? _navigateBack;
    private IosPdfDocument? _document;
    ...
    public IosPdfViewModel(string filePath, Action? navigateBack)
    {
    ...
    private async Task LoadDocumentAsync()
    {
        ...
        try
        {
            _document = await Task.Run(() => IosPdfDocument.TryOpen(_filePath));

            if (_document is null)
            {
                ErrorMessage = "Unable to open PDF on iOS";
                return;
            }
        ...
        catch (Exception ex)
        {
            ...
            Debug.WriteLine($"IosPdfViewModel: LoadDocumentAsync failed - {ex}");
        }
        finally
        {
            IsBusy = false;
    ...
    private async Task LoadPageAsync(int targetIndex)
    {
        ...
        {
            var zoomFactor = Zoom;
            var (uiImage, resolvedIndex) = await RenderIosBitmapAsync(targetIndex, zoomFactor);

            await Dispatcher.UIThread.InvokeAsync(() =>
            {
                using (uiImage)
                {
                    var avaloniaBitmap = uiImage.ToAvaloniaBitmap() ??
        ...
        {
            ErrorMessage = ex.ToString();
            Debug.WriteLine($"IosPdfViewModel: LoadPageAsync failed - {ex}");
        }
        finally
        {
            IsBusy = false;
    ...
    private Task<(UIImage image, int index)> RenderIosBitmapAsync(int requestedIndex, double zoom)
    {
        if (_document is null)
        {
        ...
        return InvokeOnUIKitThreadAsync(() =>
        {
            var pageIndex = Math.Clamp(requestedIndex, 0, Math.Max(0, _document.PageCount - 1));
            var image = _document.RenderPage(pageIndex, zoom);
            return (image, pageIndex);
        });
    }
    ...
    private void SetCurrentPage(AvaloniaBitmap? newImage)
    {
        ...
        {
            previous?.Dispose();
            Debug.WriteLine($"IosPdfViewModel: Updated page image to {newImage?.PixelSize.Width}x{newImage?.PixelSize.Height}");
    ...
    {
        return _document is not null && index >= 0 && index < PageCount;
    }
    ...
    private async Task EnsureThumbnailsAsync()
    {
        ...
        _thumbnailWarmupTask = Task.Run(async () =>
        {
            ...
            for (var i = 0; i < _document.PageCount; i++)
            {
                ...
                var (uiImage, resolvedIndex) = await RenderIosBitmapAsync(i, 0.3);
    ...
    private static Task<T> InvokeOnUIKitThreadAsync<T>(Func<T> action)
    {
        if (NSThread.IsMain)
        {
