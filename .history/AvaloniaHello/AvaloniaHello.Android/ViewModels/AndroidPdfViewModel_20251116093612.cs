...
internal sealed class AndroidPdfViewModel : ViewModelBase, IPdfDocumentViewModel, IDisposable
{
    ...
    private async Task LoadPageAsync(int targetIndex)
    {
        ...
        try
        {
            var (androidBitmap, resolvedIndex) = await Task.Run(() => RenderAndroidBitmap(targetIndex));
    ...
    private (global::Android.Graphics.Bitmap bitmap, int index) RenderAndroidBitmap(int requestedIndex)
    {
        if (_document is null)
        {
        ...
        var bitmap = _document.RenderPage(pageIndex, 1.4f);
