...
Clarifying how the proposed platform presenters interact with your existing Skia/OpenGL surfaces.
Plan: 1) explain how Avalonia renders via Skia today, 2) describe how desktop MuPDF vs Android/iOS presenters hook in, 3) confirm functional behavior and layout considerations.

Avalonia’s shared UI still renders through Skia/OpenGL on every platform; nothing in the presenter abstraction changes that baseline.
Desktop stays exactly as now: the MuPDF renderer produces bitmaps and Skia scenes consume them, so your existing rendering path remains functional.
On Android/iOS, the presenter approach hosts the OS PDF widget (Jetpack PdfRendererView, iOS PDFView/QLPreview) beside your Skia tree using Avalonia’s native-view embedding APIs. Skia continues to draw the rest of the UI; the PDF view is just another native control layered within the same visual tree.
Because those OS viewers are native, they bypass Skia for their own pixels, but Avalonia handles layout/input dispatch so the experience is seamless—scroll, zoom, annotations all work as the platform provides.
Net effect: you keep one shared Avalonia layout, Skia still drives your custom controls, and the embedded PDF components remain fully functional without you maintaining separate Xamarin UI stacks.

## Pdf presenter architecture (2025-11-16)

- Added `IPdfPresenter` + `PdfPresenter.Current` so every platform chooses an embedded viewmodel vs an external launcher through a single abstraction.
- Desktop keeps using `PdfViewerViewModel`/MuPDF via `DesktopPdfPresenter`, while mobile defaults to `LauncherOnlyPdfPresenter` until a native host (Jetpack/PDFKit) is supplied.
- `AppViewModel` now asks the presenter for an embedded viewmodel and only falls back to `TryPresentExternallyAsync` if the platform cannot render in-process.
- Android head configures both `PdfLauncher` and `PdfPresenter` during `MainActivity.OnCreate`, so no stray MuPDF bits remain there.
- Android now ships an embedded presenter: `AndroidPdfPresenter` renders pages via `Android.Graphics.Pdf.PdfRenderer`, exposes them through `AndroidPdfViewModel`, and presents them inside the new `AndroidPdfView` (Avalonia UI). Navigation/zoom is basic (per-page pager) but entirely in-app.
- Verify on device/emulator:
	1. `dotnet build AvaloniaHello.Android/AvaloniaHello.Android.csproj -c Debug`
	2. Deploy with `dotnet publish -f net8.0-android /t:Run` or VS/ADB, tap **Open GDL90 PDF**, use ◀ ▶ to flip pages; confirm no external intent opens.
- Pending: create `iOSPdfPresenter` using `PDFKit`, and update Android presenter to add zoom/scroll + caching if needed.